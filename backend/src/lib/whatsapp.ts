...
export async function sendWhatsAppMessage(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error(
        `[whatsapp] Send failed (HTTP ${response.status}, code ${body.error?.code}):`,
        body.error?.message,
      );
...
export async function sendBookingEnquiryWhatsApp(
  ...
  if (!isWhatsAppConfigured() || !notifyTo) {
    console.warn(
      "[whatsapp] Skipped — WHATSAPP_ACCESS_TOKEN, WHATSAPP_PHONE_NUMBER_ID, or WHATSAPP_NOTIFY_TO is not set.",
    );
    return { sent: false, reason: "not_configured" };
  }
...
function renderEnquiryText(data: BookingEnquiryInput): string {
  return [
...
function templateSafe(value: string): string {
